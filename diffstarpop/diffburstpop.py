...
@jjit
def _get_unbounded_lgfburst_param(u_param, bound):
    lo, hi = bound
    mid = 0.5 * (lo + hi)
    return _inverse_sigmoid(u_param, mid, 0.1, lo, hi)
...
def _get_bounded_lgfburst_params(u_params):
    return _get_lgfburst_params_kern(u_params, LGFBURST_BOUNDS)


@jjit
def _get_unbounded_lgfburst_params(params):
    return _get_lgfburst_u_params_kern(params, LGFBURST_BOUNDS)
