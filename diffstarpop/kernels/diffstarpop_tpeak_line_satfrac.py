...
from .sfh_pdf_tpeak_line_satfrac import _sfh_pdf_scalar_kernel
...
def mc_diffstar_u_params_singlegal_kernel(
    ...
    frac_quench_cen, frac_quench_sat = means_covs[:2]
    frac_quench = jnp.where(upid == -1, frac_quench_cen, frac_quench_sat)
    mu = means_covs[2]
    mu_ms_block = mu[:4]
    mu_qseq_block = mu[4:]
    cov_qseq_ms_block = means_covs[3]
    cov_qseq_q_block = means_covs[4]
