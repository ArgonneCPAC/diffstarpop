...
compute_diff_histograms_mstar_ssfr_atmobs_z_vmap = jjit(vmap(compute_diff_histograms_mstar_ssfr_atz, in_axes=_A))
...
def mstar_ssfr_sat_kern_tobs(u_params, loss_data):
    ...
    pdfs_z0 = pred_mstar_ssfr_pdf[0:11].reshape((11, nms, nsf))
    pdfs_z1 = pred_mstar_ssfr_pdf[11:22].reshape((11, nms, nsf))
    pdfs_z2 = pred_mstar_ssfr_pdf[22:32].reshape((10, nms, nsf))
    pdfs_z3 = pred_mstar_ssfr_pdf[32:42].reshape((10, nms, nsf))
    pdfs_z4 = pred_mstar_ssfr_pdf[42:52].reshape((10, nms, nsf))
    """
    pdfs_z0 = pred_mstar_ssfr_pdf[0:8].reshape((8, nms, nsf))
    pdfs_z1 = pred_mstar_ssfr_pdf[8:16].reshape((8, nms, nsf))
    pdfs_z2 = pred_mstar_ssfr_pdf[16:24].reshape((8, nms, nsf))
...
loss_mstar_ssfr_sat_kern_tobs_grad_kern = jjit(value_and_grad(loss_mstar_ssfr_sat_kern_tobs, argnums=(0,)))
...
def loss_combined_wrapper(flat_uparams, loss_data_mstar, loss_data_ssfr_cen):
    ...
    namedtuple_uparams = array_to_tuple_new_diffstarpop(flat_uparams, UnboundParams)
    loss, grads = loss_combined_grad_kern(namedtuple_uparams, loss_data_mstar, loss_data_ssfr_cen)
    grads = tuple_to_jax_array(grads)

    return loss, grads

@jjit
def loss_combined_3loss_kern(u_params, loss_data_mstar, loss_data_ssfr_cen, loss_data_ssfr_sat):
    loss_mstar_ssfr_val_cen = loss_mstar_ssfr_kern_tobs(u_params, loss_data_ssfr_cen)
    loss_mstar_ssfr_val_sat = loss_mstar_ssfr_sat_kern_tobs(u_params, loss_data_ssfr_sat)
    loss_mstar_val = loss_mstar_kern_tobs(u_params, loss_data_mstar)

    return loss_mstar_ssfr_val_cen + loss_mstar_val + loss_mstar_ssfr_val_sat

loss_combined_3loss_grad_kern = jjit(value_and_grad(loss_combined_3loss_kern, argnums=(0,)))

def loss_combined_3loss_wrapper(flat_uparams, loss_data_mstar, loss_data_ssfr_cen, loss_data_ssfr_sat):
        
    namedtuple_uparams = array_to_tuple_new_diffstarpop(flat_uparams, UnboundParams)
    loss, grads = loss_combined_3loss_grad_kern(namedtuple_uparams, loss_data_mstar, loss_data_ssfr_cen, loss_data_ssfr_sat)
    grads = tuple_to_jax_array(grads)
