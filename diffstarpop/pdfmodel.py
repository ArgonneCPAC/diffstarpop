...
from .star_wrappers import compute_histories_on_grids
...
def get_sfh_param_grid_Q(pdf_model_pars, logm0_bins, sfh_lh_sig, n_sfh_param_grid):
    means_logm0_bins, covs_logm0_bins = get_binned_means_and_covs_Q(
...
def get_param_grids_Q(
    ...
    sfh_param_grids = get_sfh_param_grid_Q(
...
def _get_default_pdf_SFH_prediction_quench(
    ...
    dmhdt_grids, log_mah_grids, sfh_param_grids = get_param_grids_Q(
        t_table,
        n_halos_per_bin,
        jran_key,
        logm0_binmids,
    ...
    means, covs = get_binned_means_and_covs_Q(pdf_model_params, logm0_binmids)
